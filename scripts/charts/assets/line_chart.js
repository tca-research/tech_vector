...
function renderLineChart() {
  ...
  bindTooltipHover(hit, (ev) => {
    ...
    // data.dates entries are ISO ("YYYY-MM-DD"/"YYYY-MM") per DATA_FORMAT.md,
    // not a display string — unlike interactive_line.js's "Month Year" dates,
    // which are already human-readable. Format for display here instead of
    // showing the raw ISO string in the tooltip title.
    const dateLabel = new Date(dates[idx]).toLocaleDateString("en-AU", { year: "numeric", month: "long" });
    showTooltip(ev.clientX, ev.clientY, ttBox(dateLabel, [ttRow(color, cfg.endLabel || cfg.title, formatValue(data.values[idx]))]));
